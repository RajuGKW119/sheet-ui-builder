...
import {
  FileSpreadsheet, BarChart2, Table2, ArrowLeft,
  Rows3, Columns3, Info, Hash, Calendar, Tag,
  Link, CheckSquare, Type, Sparkles, MessageSquare, LayoutGrid,
} from 'lucide-react'
import { ParsedData } from '@/lib/parsers'
import { analyzeAllColumns, ColumnInfo, TYPE_COLORS } from '@/lib/columnDetector'
...
type Tab = 'table' | 'charts' | 'insights' | 'chat' | 'pivot' | 'summary'
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
      ...
      <div className="space-y-1.5 text-xs text-slate-500">
        {col.type === 'number' && col.min !== undefined && (
          <>
            <div className="flex justify-between"><span>Min</span><span className="font-mono text-slate-700">{col.min?.toLocaleString()}</span></div>
            <div className="flex justify-between"><span>Max</span><span className="font-mono text-slate-700">{col.max?.toLocaleString()}</span></div>
            <div className="flex justify-between"><span>Avg</span><span className="font-mono text-slate-700">{col.mean?.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
          </>
        )}
        {(col.type === 'category' || col.type === 'boolean') && (
          <>
            <div className="flex justify-between"><span>Unique values</span><span className="font-mono text-slate-700">{col.uniqueCount}</span></div>
            {col.uniqueValues && (
              <div className="flex flex-wrap gap-1 mt-1">
                {col.uniqueValues.slice(0, 5).map(v => (
                  <span key={v} className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded text-xs truncate max-w-[120px]" title={v}>{v}</span>
                ))}
                {(col.uniqueCount || 0) > 5 && <span className="text-slate-400 text-xs">+{(col.uniqueCount || 0) - 5} more</span>}
              </div>
            )}
          </>
        )}
...
export default function DataViewer({ data, onReset }: DataViewerProps) {
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode; badge?: string }[] = [
    ...
    { id: 'pivot', label: 'Pivot', icon: <LayoutGrid className="w-4 h-4" /> },
    { id: 'insights', label: 'AI Insights', icon: <Sparkles className="w-4 h-4" />, badge: 'AI' },
    { id: 'chat', label: 'Chat', icon: <MessageSquare className="w-4 h-4" />, badge: 'AI' },
    { id: 'summary', label: 'Columns', icon: <Info className="w-4 h-4" /> },
  ]
    ...
    <div className="min-h-screen bg-slate-50">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              {data.sheetTitle && data.sheetTitle !== 'Google Sheet' && (
                <span className="text-slate-400 text-sm truncate hidden sm:block">· {data.sheetTitle}</span>
              )}
            </div>
          </div>
          <div className="hidden sm:flex items-center gap-3 text-sm text-slate-500 flex-shrink-0">
            <span className="flex items-center gap-1"><Rows3 className="w-3.5 h-3.5" />{data.rows.length.toLocaleString()} rows</span>
            <span className="flex items-center gap-1"><Columns3 className="w-3.5 h-3.5" />{data.headers.length} cols</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          ...
          <StatPill label="Numeric Cols" value={columns.filter(c => c.type === 'number').length} accent="text-emerald-600" />
          <StatPill label="Category Cols" value={columns.filter(c => c.type === 'category' || c.type === 'boolean').length} accent="text-purple-600" />
        </div>

        {/* Tabs */}
        <div className="flex flex-wrap gap-1 bg-white border border-slate-200 rounded-xl p-1 w-fit mb-6 shadow-sm">
          {tabs.map(tab => (
            <button
              key={tab.id}
        ...
        <div className="animate-fade-in">
          ...
          {activeTab === 'pivot' && (
            <PivotTable headers={data.headers} rows={data.rows} columns={columns} />
          )}
          {activeTab === 'insights' && (
            <InsightsPanel headers={data.headers} rows={data.rows} columns={columns} />
          )}
          {activeTab === 'chat' && (
            <AIChat headers={data.headers} rows={data.rows} columns={columns} />
          )}
          {activeTab === 'summary' && (
